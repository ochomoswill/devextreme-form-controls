...
type CleansedDxRangePickerProps = Omit<DxRangePickerProps, 'onChange' | 'onFocusOut' | 'startDateValidationStatus' | 'endDateValidationStatus'>;
export type RHFRangePickerProps = OwnProps & UseControllerProps<any> & CleansedDxRangePickerProps;

const RHFRangePicker: FunctionComponent<RHFRangePickerProps> = (props) => {
    ...
    if (fieldState.invalid) {
        ...
        if (fieldState?.error?.type === 'startDateRequired') {
            startDateValidationStatus = 'invalid'
            startDateValidationErrors = [{message: fieldState?.error?.message}]
        }
